...
- **ctx XX%** — current context window usage
- **5h XX%** — 5-hour billing window remaining
- **7d XX%** — 7-day window remaining
- **$X.XX / XXk tok** — session-to-date cost and tokens

The 5h/7d numbers come from Anthropic's official `usage` endpoint (the same data shown in `/status`), not from third-party token counters.
